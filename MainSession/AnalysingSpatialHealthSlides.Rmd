...
\mathbf{v} & \sim \mathsf{N}(0, \sigma^2_v),\\
\mathbf{u} & \sim \mathsf{H}
\end{align*}
where $H$ is some distribution with spatial structure.
- Classical choice for $H$ is the intrinsic ICAR, specified by a set of conditionals
\begin{align*}
...
in Bayesian hierarchical models with a latent Gaussian component: 
\begin{align*}
...
- Model syntax is similar to `lm` or `glm` with latent models encoded using `f(, model = )`
  - `f(, mode = 'iid')` for v.
  - `f(, mode = 'besag')` for u.
  - `f(, mode = 'bym')` to do both all in one.
